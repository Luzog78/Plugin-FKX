...
public class Lightning implements CommandExecutor, TabCompleter {
    public static final String syntaxe = "/lightning (-rod | -pos <x> <y> <z> | <players...>)";
            ...
            if (args[0].equalsIgnoreCase("-rod"))
                ...
                if (args.length >= 4)
                    try {
                        ...
                        w.strikeLightning(new Location(w, Double.parseDouble(args[1]), Double.parseDouble(args[2]), Double.parseDouble(args[3])));
                ...
                CmdUtils.getPlayersFromArray(args, 0).forEach(player -> {
                    ...
                    u.succ("§6" + player.getDisplayName() + "§r a été §efoudroyé§r !");
                });
