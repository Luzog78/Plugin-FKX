...
public class GuiPerm {
    ...
    public static Inventory getInv(String back) {
        ...
        inv.setItem(Utils.posOf(4, 2),
                getPermsItem(fk.getPriority(), Material.BEACON, "§bPrioritaires", "§7Clic pour voir plus", "fk perm priority"));
        inv.setItem(Utils.posOf(2, 3),
                getPermsItem(fk.getGlobal(), Material.BEDROCK, "§fGlobal", "§7Clic pour voir plus", "fk perm global"));
        inv.setItem(Utils.posOf(3, 4),
                getPermsItem(fk.getNeutral(), Material.GRASS, "§fZones Neutres", "§7Clic pour voir plus", "fk perm neutral"));
        inv.setItem(Utils.posOf(5, 4),
                getPermsItem(fk.getFriendly(), Material.APPLE, "§fZones Amicales", "§7Clic pour voir plus", "fk perm friendly"));
        inv.setItem(Utils.posOf(6, 3),
                getPermsItem(fk.getHostile(), Material.DIAMOND_SWORD, "§fZones Hostiles", "§7Clic pour voir plus", "fk perm hostile"));
    ...
    public static Inventory getPermsInv(FKPermissions perms, ItemStack main, ItemStack second, String commandSpecification, String back) {
        ...
        int i = 1;
        for (FKPermissions.Type t : FKPermissions.Type.values()) {
            Items.Builder b = Items.builder(perms.getPermission(t) == FKPermissions.Definition.ON ? Items.lime()
                            : perms.getPermission(t) == FKPermissions.Definition.OFF ? Items.red()
                            : Items.gray())
                    .setName("§6" + t.name() + " :  §f" + perms.getPermission(t).toFormattedString())
                    .setLore("§8" + Guis.loreSeparator, "§7Clic Gauche pour ON / OFF", "§7Clic Droit pour DEFAULT")
                    .setCantClickOn(true)
                    .setLeftRightCommandOnClick("fk perm " + commandSpecification + " " + t.name() + " "
                            ...
                            "fk perm " + commandSpecification + " " + t.name() + " default"
                                    + "\nfk perm " + commandSpecification);
